...
### Manual work membership (issue #143, real library + both themes)

**Manage editions** is a disclosure whose whole point is the state *after* a
mutation, so these are hand-captured from a real library too. They document the
flow that automatic grouping cannot serve: `Nicomachean Ethics` (ebook) and
...
metadata. Linking them is the override issue #143 asks for.
...
Automatic grouping stays the default. **Manage editions** is the explicit
override for a wrong automatic grouping, and it is a separate rail element
rather than part of the Available Formats & Editions block: a book alone in its
work has no editions block, and that is exactly the book that needs to be linked
to another one.

- **Collapsed by default, and it fetches nothing while collapsed.** The candidate
  list is loaded only when the disclosure is opened, so an ordinary book-detail
  visit costs no extra query. The member list and the picker are not in the DOM
  until then — "secondary" is structural, not just visual.
