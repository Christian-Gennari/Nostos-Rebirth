...
- **Tracking:** #438, Wave 0 + Wave 1
- **Phase-1 inventory origin:** `main` at `b8e94be405294ea4245030ed006b179e0c763927`
- **Hardened extraction baseline:** `main` at `e20df0daf352cb94bec9867dbe3b78c64678f83b` (PR #458 / #408)
- **Date:** 2026-09-23
- **Scope:** live public/private classification used by #459 and the later extraction waves
- **Coordination:** #408 is merged and its formerly blocked surfaces have been re-read against the hardened baseline.

## Wave 0 post-#408 reconciliation

PR #458 merged #408 at `e20df0daf352cb94bec9867dbe3b78c64678f83b`. That SHA is the extraction baseline for #459 and every later private-host migration wave.

The Phase-1 classifications still hold, with these post-hardening clarifications:

- **Hosted privacy lifecycle is B-class.** #408 added `Cloud/Privacy/CloudAccountDeletion*.cs`, `Configuration/CloudAccountDeletionRegistration.cs`, and `Endpoints/CloudAccountDeletionEndpoints.cs`. The 14-day recoverable deletion lifecycle, destructive resource cleanup worker, and hosted account-state orchestration belong to the future private hosted layer. The public #399 archive format/service remains A-class.
- **Product endpoints temporarily gained hosted policy hooks.** #408 added Cloud rate-limit metadata to Books/Provider/Portability and a narrow recoverable-account export rule. #459 keeps those endpoint implementations public but replaces direct hosted dependencies with opaque host policy names plus an `IPortableArchiveExporter` seam. The hosted deletion-grace behavior now sits in `CloudPortableArchiveExporter`.
- **Upload hardening remains product-safe.** MIME/extension checks and the 25 MiB cover bounds protect the common product surface. #459 therefore makes the cover limits public product constants while Cloud continues to reference the same values.
- **Health and Data Protection remain split surfaces.** Generic response/SelfHosted SQLite readiness and ordinary Data Protection are public; Cloud control-plane/object-storage readiness and control-plane key persistence are hosted composition.
- **Auth/tenant authority is unchanged.** The hardened chain remains validated identity -> canonical `NostosAccountId` -> trusted control-plane mapping -> customer PostgreSQL/B2/commercial resources. No product contract accepts browser-supplied tenant authority.
- **Cloud operational hardening stays hosted.** HSTS, account-partitioned rate limiting, provisioning response redaction, bounded PostgreSQL pools, recovery cleanup, fleet leases, billing/provider hardening, and staging/release operations remain B-class.
- **No #408 change made the canonical product model hosted-only.** `NostosDbContext`, repositories, storage/AI contracts, assistant behavior, portability, deployment capabilities and the single Angular frontend remain public.

The short hosted-code migration freeze is active from this baseline: only security/correctness fixes should alter hosted implementation until the private host reaches parity.
...
#459 establishes the reusable public `Nostos.Product` boundary while retaining `Nostos.Backend` as the current runnable SelfHosted/transitional Cloud host:
...
For #459, product source keeps its historical paths but is compiled exactly once by `Nostos.Product`; `Nostos.Backend` references that project and removes those files from its own compile items. This avoids source duplication and mass namespace churn while giving #460 a real project boundary to consume.
...
### Dependency graph after #459

- `Nostos.Product` -> `Nostos.Shared`;
- `Nostos.Backend` -> `Nostos.Product` + `Nostos.Shared`;
- `Nostos.Backend.Tests` -> `Nostos.Backend` + `Nostos.Product` + `Nostos.Shared`;
- `Nostos.Shared` has no project references;
- `Nostos.Product` has no project/assembly reference back to `Nostos.Backend`;
- Release builds of `Nostos.Backend` still build the one public `Nostos.Frontend` and copy its browser bundle to `wwwroot`.

Architecture tests assert that canonical product types are owned by `Nostos.Product`, that the Product assembly does not reference `Nostos.Backend`, and that no `Nostos.Backend.Cloud.*` implementation type is compiled into the Product assembly.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `Nostos.Backend/Cloud/CloudCustomerConnectionFactory.cs` | Backend | **B** | Npgsql connection-string derivation for per-customer databases. | Provisioning, tenant DB, migration/recovery | Private hosted infrastructure | High | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Cloud/ControlPlane/CloudAccountResource.cs` | Backend | **B** | Hosted resource map: account -> database/storage/schema/provisioning state. Not product Library domain. | Control-plane store, provisioning, recovery | Private hosted control plane | **Very high** | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Cloud/ControlPlane/CloudControlPlaneDbContext.cs` | Backend | **B** | Hosted operational/commercial database containing resources, subscriptions, billing and managed-AI usage. | Cloud stores/bootstrap | Private hosted control plane | **Very high** | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Cloud/ControlPlane/CloudControlPlaneStore.cs` | Backend | **B** | Account resource/status store; also implements Cloud account status used by auth. | Auth, provisioning, persistence, recovery | Private hosted control plane | **Very high** | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Cloud/ControlPlane/CloudDataProtectionKeyRepository.cs` | Backend | **B** | Hosted ASP.NET Data Protection key persistence/encryption against control-plane PostgreSQL. | Cloud auth/session startup | Private hosted security infrastructure | High | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
...
| `Nostos.Backend/Cloud/Persistence/CloudTenantDbContextFactory.cs` | Backend | **B** | Resolves trusted Cloud account -> control-plane mapping -> Npgsql `NostosDbContext`. Reuses public model rather than duplicating it. | Product repositories/services in Cloud requests | Private hosted persistence adapter | **Very high** | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Cloud/Provisioning/CloudCustomerDatabaseProvisioner.cs` | Backend | **B** | Creates/initializes customer PostgreSQL resources and records state. | Provisioning/onboarding | Private hosted provisioning | **Very high** | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Cloud/Migrations/CloudTenantSchemaMigrator.cs` | Backend | **B** | Fleet/customer PostgreSQL migration orchestration and compatibility checks. | Provisioning/recovery/operator lifecycle | Private hosted DB operations | **Very high** | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Cloud/Migrations/PostgresNostosDbContext.cs` | Backend | **B** | Migration-only Npgsql context inheriting the public canonical `NostosDbContext`. This is the desired reuse direction. | EF migration generation/recovery | Private hosted PostgreSQL adapter | Medium | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Cloud/Migrations/Postgres/20260922170154_InitialCloudBaseline.cs` | Backend | **B** | PostgreSQL hosted schema migration. | Cloud migration lifecycle | Private hosted PostgreSQL migrations | High | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Cloud/Migrations/Postgres/20260922170154_InitialCloudBaseline.Designer.cs` | Backend | **B** | Generated metadata for hosted PostgreSQL migration. | EF tooling/runtime | Private hosted PostgreSQL migrations | High | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Cloud/Migrations/Postgres/20260922170207_EstablishCloudMigrationLifecycle.cs` | Backend | **B** | Hosted fleet migration lifecycle. | Cloud migrator | Private hosted PostgreSQL migrations | High | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Cloud/Migrations/Postgres/20260922170207_EstablishCloudMigrationLifecycle.Designer.cs` | Backend | **B** | Generated metadata for hosted PostgreSQL migration. | EF tooling/runtime | Private hosted PostgreSQL migrations | High | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Cloud/Migrations/Postgres/PostgresNostosDbContextModelSnapshot.cs` | Backend | **B** | Hosted PostgreSQL EF snapshot generated from the public model. | EF tooling | Private hosted PostgreSQL migrations | High | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Configuration/CloudControlPlaneOptions.cs` | Backend | **B** | Hosted PostgreSQL admin/control/customer connection configuration. | Cloud persistence/provisioning/recovery | Private hosted configuration | Medium | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Configuration/CloudPersistenceRegistration.cs` | Backend | **B** | Registers control plane, Npgsql tenant factory, entitlements, managed usage, migrations and provisioning. | Program/deployment composition | Private hosted composition | **Very high** | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
...
| Current path | Project | Class | Rationale / dependencies | Consumers | Target home | Risk | #408 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `Nostos.Backend/Security/CloudAccountIdentity.cs` | Backend | **C** | Contains server-derived `NostosAccountId`, trusted tenant-context accessors and Cloud account status. These are security-critical hosting concepts, not currently needed by SelfHosted/product code. | Auth, control plane, tenant DB/storage, background jobs | Prefer private hosted security layer. Introduce a smaller public owner-context contract only if product code later genuinely needs one. | **Very high** | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Security/CloudAuthentication.cs` | Backend | **C** | Generic OIDC primitives are used, but policies are specific to official hosted access and depend on Cloud entitlements/account state. | Cloud middleware/endpoints | Private hosted auth/composition; keep only product capability semantics public | **Very high** | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Configuration/CloudAuthOptions.cs` | Backend | **B** | Hosted OIDC/Clerk configuration including secret environment variable names. | CloudAuthentication | Private hosted configuration | High | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Configuration/CloudDataProtectionOptions.cs` | Backend | **B** | Hosted session/Data Protection key protection settings. | DataProtectionRegistration | Private hosted security configuration | High | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Configuration/DataProtectionRegistration.cs` | Backend | **C** | Mixes normal public ASP.NET Data Protection setup with Cloud control-plane key storage. | Program/auth | Split: ordinary SelfHosted registration public; Cloud key repository wiring private | High | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Endpoints/CloudAuthEndpoints.cs` | Backend | **B** | Hosted login/logout/session API. Public browser code may consume it, but backend implementation and policy are host concerns. | Shared Angular Cloud-entry flow | Private hosted API | High | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
...
| `Nostos.Backend/Cloud/Onboarding/CloudOnboardingService.cs` | Backend | **B** | Orchestrates hosted subscription, entitlement and provisioning. | Cloud onboarding endpoints | Private hosted service | High | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
...
| `Nostos.Backend/Endpoints/CloudOnboardingEndpoints.cs` | Backend | **B** | Hosted account onboarding orchestration API. | Shared Angular frontend | Private hosted API | High | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Endpoints/CloudProvisioningEndpoints.cs` | Backend | **B** | Low-level hosted provisioning API. #408 explicitly needs to reconsider exposure of schema details. | Smoke tests/operator/onboarding legacy path | Private hosted API | **Very high** | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
...
| `Nostos.Backend/Services/Ai/ManagedAiAccessPolicy.cs` | Backend | **C** | File mixes public interface + SelfHosted policy with a Cloud policy that imports hosted entitlements. | Assistant endpoints/orchestrator | Split: interface/SelfHosted public; Cloud policy private | Medium | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
...
| `Nostos.Backend/Cloud/Ai/CloudAiUsageModels.cs` | Backend | **B** | Per-account managed provider usage/reservations in control plane. | Cloud usage service | Private hosted commercial/ops layer | High | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Cloud/Ai/CloudManagedAiUsageService.cs` | Backend | **B** | Enforces hosted allowance + global operator budgets. | Assistant/STT managed providers | Private hosted commercial/ops layer | High | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Configuration/CloudManagedAiOptions.cs` | Backend | **B** | Hosted provider endpoints/models/secret variable names. | Cloud managed providers | Private hosted configuration | Medium | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Configuration/CloudManagedAiUsageOptions.cs` | Backend | **B** | Operator budget/usage policy. | Cloud usage service | Private hosted configuration | Medium | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Endpoints/CloudManagedAiUsageEndpoints.cs` | Backend | **B** | Hosted usage status API; intentionally hides provider/cost internals. | Shared Settings UI | Private hosted API | Medium | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |

## 7. Object storage

| Current path | Project | Class | Rationale / dependencies | Consumers | Target home | Risk | #408 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `Nostos.Backend/Cloud/Storage/S3BookAssetStorage.cs` | Backend | **B** | S3-compatible tenant-scoped implementation of public `IBookAssetStorage`; derives namespace from trusted Cloud context/control plane. | Product media endpoints/portability | Private hosted storage adapter | **Very high** | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Cloud/Storage/CloudObjectStorageBootstrapper.cs` | Backend | **B** | Hosted bucket readiness/access validation. | Startup/health | Private hosted infrastructure | High | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Configuration/CloudObjectStorageOptions.cs` | Backend | **B** | Hosted S3/B2 endpoint/bucket/credential variable configuration. | S3 registration/storage/recovery | Private hosted configuration | High | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Configuration/CloudObjectStorageRegistration.cs` | Backend | **B** | Wires S3 client, asset storage and Cloud recovery services. | Program | Private hosted composition | **Very high** | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
...
| Current path | Project | Class | Rationale / dependencies | Consumers | Target home | Risk | #408 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `Nostos.Backend/Cloud/Recovery/CloudRecoveryModels.cs` | Backend | **B** | Hosted operational backup/restore resource manifests and audit state. Depends on public portability format but is not customer portability itself. | Recovery services/endpoints | Private hosted recovery | High | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Cloud/Recovery/CloudRecoveryService.cs` | Backend | **B** | Account-scoped staged restore orchestration over control plane + public portability. | Recovery endpoints/scheduled backup | Private hosted recovery | **Very high** | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Cloud/Recovery/CloudRecoveryControlPlane.cs` | Backend | **B** | Atomic switch of account resource mapping to staged DB/storage. | CloudRecoveryService | Private hosted recovery/control plane | **Very high** | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Cloud/Recovery/CloudRecoveryResourceManager.cs` | Backend | **B** | Creates temporary PostgreSQL/S3 restore resources and composes public archive import. | CloudRecoveryService | Private hosted recovery infrastructure | **Very high** | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Cloud/Recovery/CloudRecoveryStore.cs` | Backend | **B** | S3 operational backup store and restore audit metadata. | CloudRecoveryService | Private hosted recovery infrastructure | High | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Cloud/Recovery/CloudBackupSweepRunner.cs` | Backend | **B** | Fleet loop over eligible hosted accounts with trusted background tenant scopes. | Scheduled worker/operator execution | Private hosted operations | High | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Cloud/Recovery/CloudScheduledBackupWorker.cs` | Backend | **B** | Hosted scheduled fleet backup worker. | Cloud host | Private hosted operations | High | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Cloud/Runtime/CloudWorkerLease.cs` | Backend | **B** | PostgreSQL advisory-lock style fleet singleton lease. | Billing/recovery workers | Private hosted runtime | High | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Configuration/CloudRecoveryScheduleOptions.cs` | Backend | **B** | Hosted operator schedule. | Scheduled backup registration | Private hosted configuration | Medium | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Configuration/CloudRecoveryScheduleRegistration.cs` | Backend | **B** | Hosted worker composition. | Program | Private hosted composition | Medium | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Endpoints/CloudRecoveryEndpoints.cs` | Backend | **B** | Hosted operational recovery API. | Operator/customer recovery UX if exposed | Private hosted API | High | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `Nostos.Backend/Health/NostosHealthChecks.cs` | Backend | **C** | One file mixes generic response + SelfHosted DB readiness with Cloud control-plane/object-storage readiness. | /health/live, /health/ready | Split: generic/SelfHosted public, Cloud readiness private | High | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
...
| --- | --- | --- | --- | --- |
| `Nostos.Backend/appsettings.json` | **C** | Keep public product/SelfHosted-safe defaults; hosted production values live in private deployment configuration/environment | Medium | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
...
| `.github/workflows/postgresql-compatibility.yml` | **B/C** | Hosted Npgsql/fleet integration moves private. Provider-neutral EF-model regression remains public where possible. | High | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `.github/workflows/object-storage-integration.yml` | **B** | Hosted S3/B2 adapter integration (MinIO in CI). | Medium | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `.github/workflows/cloud-recovery-integration.yml` | **B** | Hosted PostgreSQL+S3 staged recovery. | High | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
...
| `.github/workflows/cloud-runtime.yml` | **C** | Contains useful generic/SelfHosted production-container checks plus Cloud runtime behavior. Split: public container/SelfHosted checks, private Cloud runtime checks. | High | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `.github/workflows/cloud-release.yml` | **B** | Builds/publishes official hosted immutable image and staging smoke path. Future private hosted release pipeline. | High | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
| `scripts/cloud/staging-smoke.sh` | **B** | Official Cloud liveness/readiness/capability/provisioning smoke. | Private hosted ops | **RECONCILED #408** — reviewed against `e20df0d`; classification unchanged unless the Wave 0 notes above say otherwise. |
