...
| **Clerk** | Managed Auth & Accounts | `contact@cgennari.com` (Developer / Dashboard Admin) | App `Nostos` (`app_3Jh5c9J9bK3MAPXJYLFJhNQHLS8`), Instance `ins_3Jh5c2V8AjyTkk8RYcFcbIAjNpq` | `NOSTOS_CLOUD_AUTH_CLIENT_SECRET` | Email / OTP / OAuth | Active (Alpha) |
...
- **Workspace:** `org_3Jh5VTGilAUsJCDaALEg5dgJagp`.
- **Primary Owner / Developer:** `contact@cgennari.com`.
- **Runtime Secret:** `NOSTOS_CLOUD_AUTH_CLIENT_SECRET`.
- **Ownership Notes:** Dashboard team administration is tied to Christian's developer account. Instance-level organization feature is not enabled for end-users, preserving single-user library semantics. Dashboard co-administration can be invited via Clerk Dashboard > Settings > Members when multi-operator dashboard access is desired. Documented as an accepted developer boundary for alpha.
